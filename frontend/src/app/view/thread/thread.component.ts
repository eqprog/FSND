...
export class ThreadComponent implements OnInit {
  ...
  public confirmDelete(post: Post): void {
    ...
    this.http.delete<{status: 'SUCCESS' | 'FAILURE'}>(`http://127.0.0.1:5000/threads/${this.thread?.id}`, {headers: this.auth.getHeaders(), body: { post_id: post.id }}).pipe((take(1))).subscribe({ next: (response: { status: 'SUCCESS' | 'FAILURE' }) => {
