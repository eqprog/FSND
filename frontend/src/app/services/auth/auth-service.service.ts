...
export class AuthService {
  ...
  authHeaders = new HttpHeaders().set('Content-Type', 'application/json')
  ...
  decodeJWT(token: string) {
    ...
    this.authHeaders.set('Authorization', `Bearer: ${this.activeJWT()}`);
  ...
  getHeaders(): Record<string, string> {
    ...
    if (this.activeJWT()) {
      headers['Authorization'] = `Bearer ${this.activeJWT()}`
    }
    return headers;
  }
