...
export class ForumService {
  ...
  public getForum(id: string): void {
    ...
    this.loading = true;
    this.responseSubscription = this.http.get<ForumResponse>(`${this.apiUrl}/forum/${id}`).pipe(take(1)).subscribe((response: ForumResponse) => {
  ...
  public postForumThread(form: FormGroup): void {
    ...
    this.http.post(`${this.apiUrl}/forum/${state.forum.id}`, { ...form.value, user_id: 2 }, {
