...
export class AdminComponent implements OnInit {
  ...
  protected getUserList(): void {
    const subscription = this.http.get<UserListResponse>('${this.apiUrl}/admin/users', { headers: this.auth.getHeaders()}).pipe(take(1)).subscribe((response: UserListResponse) => {
  ...
  private submitCreateForum(requestBody: {}): void {
    ...
    const subscription = this.http.post<StatusMessage>('${this.apiUrl}/admin/create/forum', requestBody, {
  ...
  private submitUserBan(banRequest: any): void {
    ...
    const subscription = this.http.post<StatusMessage>('${this.apiUrl}/admin/ban-user', request, {
